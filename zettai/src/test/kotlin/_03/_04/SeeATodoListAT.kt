...
class SeeATodoListAT {
    ...
    @Test
    fun `List owners can see their lists with ddt`() {
        val apps = listOf(
    ...
    fun startTheApplicationDomainOnly(lists: Map<User, List<ToDoList>>): ApplicationForAT {
        val port = 8081
        val server = Zettai(ToDoListHub(lists)).asServer(Jetty(port))
        server.start()
        val client = ClientFilters
    ...
    fun startTheApplicationHttp(lists: Map<User, List<ToDoList>>): ApplicationForAT {
        val port = 8081
        val server = Zettai(ToDoListHub(lists)).asServer(Jetty(port))
        server.start()
        val client = ClientFilters
